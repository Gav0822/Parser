...
public class Parser {
    ...
    public void expr_prime() {
       ...
       /***********************************
        * should comment out debugging statements
        */
       if (match(token, Token.Type.ADDITION)) {
            nextToken();
            term();
            expr_prime();
        } else if (match(token, Token.Type.SUBTRACTION)) {
            nextToken();
            term();
            expr_prime();
        }
    ...
    public void term_prime() {
        ...
        if (match(token, Token.Type.MULTIPLICATION)) {
            nextToken();
            factor();
            term_prime();
        } else if (match(token, Token.Type.DIVISION)) {
            nextToken();
            factor();
            term_prime();
        }
    ...
    public void factor() {
        ...
        if (match(token, Token.Type.L_PAREN)) {
            printNextToken();
            nextToken();
            expr();
            if (!match(token, Token.Type.R_PAREN)) {
                System.err.println(token.tokenError(Token.Type.R_PAREN));

               /***************************************
                * would be better to have this logic
                * in match
                */
               this.error = true;
            }
    ...
    public void number() {
        ...
        if (!match(token, Token.Type.INT_LITERAL)) {
     ...
     */
    public void parse() {
        while (!match(token, Token.Type.EOS)) {
            this.expr();
        }
    }
    ...
    public boolean match(Token u, Token.Type type){
        return u.getType().equals(type);
    }
}
